...
def main():
    ...
    for j in range(len(layout[0])):
        visited = travel(layout, Cords(0, j), Direction.DOWN)
        energized.append(len(set([cords for cords, _ in visited])))

        visited = travel(layout, Cords(len(layout) - 1, j), Direction.UP)
        energized.append(len(set([cords for cords, _ in visited])))

    for i in range(len(layout)):
        visited = travel(layout, Cords(i, 0), Direction.RIGHT)
        energized.append(len(set([cords for cords, _ in visited])))

        visited = travel(layout, Cords(i, len(layout[0]) - 1), Direction.LEFT)
        energized.append(len(set([cords for cords, _ in visited])))
